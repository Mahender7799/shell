...
do 
   ...
   dnf list installed $package &>>LOGG_FILE
   
   #if exit status 0, already installed elase not installed
   if [$? -ne 0];then
       dnf install $package -y &>>LOGG_FILE
       VALIDATE $? "$package"
   else
      echo -e "$package is alreasy installed ... $Y SKIPPING$N"
